...
def destroyer(root):
    if messagebox.askquestion("Quit", "Certeza?") == "yes":
        root.quit()
        root.destroy()
        sys.exit()

def method_change(all_widgets, method_name, methods_list, out, reset_btn, enter_btn, exp_entry):
    ...
    reset(out, reset_btn, enter_btn, exp_entry)
...
def confirm(out,numofeqns, reset_btn, enter_btn, exp_entry):
...
def enter_eqn(out, equation, enter_btn, exp_entry):
    global EQUATIONS
    global COUNTER
    global NUMBER_OF_EQUATIONS
    print(COUNTER)
    if equation != '' and COUNTER > 0:
        try:
            ...
            COUNTER -= 1
            update_output(out, "Equação " + str(NUMBER_OF_EQUATIONS - COUNTER) + ": " + equation + "\n", append=True)
...
def calc (out, method):
    ...
    if NUMBER_OF_EQUATIONS == 0:
        update_output(out, "Confirme o número de equações!\n", color="red", append=True)
        return
    if len(EQUATIONS) != NUMBER_OF_EQUATIONS:
        update_output(out, "Entre com todas as equações!\n", color="red", append=True)
        return
    if method == "All" or method == "Gauss seidel":
        if len(INIT_VALS) != NUMBER_OF_EQUATIONS:
            update_output(out, "Entre com todos os valores iniciais!\n", color="red", append=True)
            return
    info = {
        'method': method,
...
def out_methods(out, results, method):
    ...
    update_output(out, "Solução: \n", append=True)
    print_sol(out, results['solution'])
    update_output(out, "Tempo de processamento: " + str(results['time']) + "\n", append=True)
